...
class Database {
 public:
  ...
  using LweVector64 = std::vector<lwe::Integer64>;
  using LweMatrix = std::vector<LweVector>;
  using LweMatrix64 = std::vector<LweVector64>;
  ...
  // Returns a fake database with random records for the given parameters.
  static absl::StatusOr<std::unique_ptr<Database>> CreateRandomFake(
      const Parameters& parameters);

  // Returns a database with records (row_id + 3*col_id) for fast database setup for the given parameters.
  static absl::StatusOr<std::unique_ptr<Database>> CreateFast(
      const Parameters& parameters);
  ...
  absl::StatusOr<std::vector<LweMatrix>> TransposeMatrixProduct(
    const LweMatrix& lwe_matrix, bool secondary = false) const;

  absl::StatusOr<std::vector<LweMatrix64>> TransposeMatrixProduct64(
    const LweMatrix64& lwe_matrix, bool secondary = false) const;

  absl::StatusOr<std::vector<LweMatrix>> DirectMatrixProduct(
    const LweMatrix& lwe_matrix, bool secondary = false) const;

  std::vector<LweMatrix> TransposeMatrixProductFake() const;

  std::vector<LweMatrix64> TransposeMatrixProductFake64() const;

  std::vector<LweMatrix> DirectMatrixProductFake() const;

  absl::StatusOr<std::vector<std::vector<LweVector>>> TransposeMatrixCiphertextProducts(
    const std::vector<LweVector>& cts) const;

  absl::StatusOr<std::vector<std::vector<LweVector64>>> TransposeMatrixCiphertextProducts64(
    const std::vector<LweVector64>& cts) const;
  ...
  //absl::StatusOr<std::vector<LweVector>> InnerProductWithFaster32by8(
  //  const LweVector& query) const;

  // Accessors.
  absl::StatusOr<std::string> Record(int64_t index) const;
  absl::StatusOr<std::string> RecordWithStacking(int64_t index) const;

  absl::Span<const RawMatrix> Data() const { return data_matrices_; }
  absl::Span<const LweMatrix> Hints() const { return hint_matrices_; }
  const lwe::Matrix* LweQueryPad() const { return lwe_query_pad_; }
 ...
 private:
  explicit Database(Parameters params, const lwe::Matrix* lwe_query_pad,
                    int64_t num_records, std::vector<RawMatrix> data_matrices, std::vector<RawMatrix> data_matrices_transpose,
                    std::vector<LweMatrix> hint_matrices)
      : params_(std::move(params)),
        lwe_query_pad_(lwe_query_pad),
        num_records_(num_records),
        data_matrices_(std::move(data_matrices)),
        data_matrices_transpose_(std::move(data_matrices_transpose)),
  ...
  // Transpose of the database matrices.
  std::vector<RawMatrix> data_matrices_transpose_;
