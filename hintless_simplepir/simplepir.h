...
// Need to return both pad (A) and hint to be passed to VeriSimplePIR
struct PublicMatrixAandH {
  lwe::Matrix pad;
  lwe::Matrix hint;
};
...
class Parems {
 public:
  ...
  // Preprocesses the database-dependent hint for the client and also return A (pad).
  absl::StatusOr<PublicMatrixAandH> ServerPreprocessGetAandH(
      const lwe::Matrix& database) const {
    if (database.cols() != DbCols()) {
      return absl::InvalidArgumentError(
          absl::StrCat("The number of cols in the database, ", database.cols(),
    ...
    RLWE_ASSIGN_OR_RETURN(
        ...
        lwe::SampleUniformMatrix(DbCols(), LweSecretDim(), pad_prng.get()));
    lwe::Matrix hint = database * pad;
    return PublicMatrixAandH{.pad = pad, .hint = hint};
  }
