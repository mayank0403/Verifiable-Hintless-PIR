...
inline std::string ReconstructRecordWithStacking(absl::Span<const lwe::Integer> values,
                                     const Parameters& params) {
  //std::cout << "Size of values " << values.size() << std::endl;
  int total_shards =
        DivAndRoundUp(params.db_record_bit_size, params.lwe_plaintext_bit_size);
  int total_stacks = params.db_stack_cells;
  if (values.size() != total_shards * total_stacks) {
    std::cout << "Size of values vector input to ReconstructRecord is incorrect" << std::endl;
    assert(false);
  }
  int num_bytes = DivAndRoundUp(params.db_record_bit_size * params.db_stack_cells, 8);
  //int num_bytes = values.size();
  std::string record(num_bytes, 0);
  auto curr_byte = record.begin();
  int num_remaining_bits = params.db_record_bit_size * params.db_stack_cells;
  for (int i = 0; i < values.size(); ++i) {
    // We use the bits in `values[i]` in two steps: first we fill in the current
    // byte of `record` with the "head" bits, and then use the remaining "body"
    // bits to fill in `record` starting from the next byte.
    lwe::Integer value = values[i];
    int num_filled_bits = (params.db_record_bit_size * params.db_stack_cells - num_remaining_bits) % 8;
    int num_shard_bits =
        std::min(num_remaining_bits, params.lwe_plaintext_bit_size);
    int num_head_bits = std::min(8 - num_filled_bits, num_shard_bits);
    lwe::Integer head_mask = (lwe::Integer{1} << num_head_bits) - 1;
...
inline Integer ConvertModulus(const Integer& x, const Integer& q,
  ...
  //std::cout << "Convert Modulus " << x << " " << q << " " << p << " " << q_half << std::endl;
  if (x > q_half) {
    //Integer diff = (q - x) % p;
    Integer diff = q - x;
    //return p - diff; //% p;
...
template <typename Integer>
inline Integer ConvertModulusDirect(const Integer& x, const Integer& q,
                              const Integer& p, const Integer& q_half) {
  //std::cout << "Convert Modulus " << x << " " << q << " " << p << " " << q_half << std::endl;
  //if (x > q_half) {
    //Integer diff = (q - x) % p;
