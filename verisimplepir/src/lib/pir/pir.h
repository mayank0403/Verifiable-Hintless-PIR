...
public:
    ...
    // Constructor invoked when params are precomputed by HintlessPIR caller
    VLHEPIR(const uint64_t row_in, const uint64_t col_in, const uint64_t lattice_n_in, const uint64_t d_in, 
        const bool allowTrivial = true, const bool verbose = false, 
        const bool simplePIR = false,
        const bool randomData = true, 
        const uint64_t batchSize = 1,
    ...
    ) : N(row_in * col_in), d(d_in), db(row_in * col_in, d_in)
    {
        ...
        uint64_t logp = d_in; 
        dbParams = db.confirmHintlessParams(N, logp, row_in, col_in, d_in, allowTrivial, verbose, simplePIR, batchSize, false, honestHint);
        lhe = LHE(dbParams.p);
        m = dbParams.m;
        ell = dbParams.ell;
        if (!simplePIR)
    ...
    Matrix EmptyA() const; // NOTE: Make sure to fill this matrix with random data before using it; we will fill this up with the A from HintlessPIR
