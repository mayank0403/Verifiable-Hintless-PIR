...
public:
    ...
    ) : N(N_in), d(d_in), db(N_in, d_in)
    {
        ...
        VeriSimplePIR(const uint64_t row_in, const uint64_t col_in, const uint64_t lattice_n_in, const uint64_t d_in, const uint64_t logp_in,
        const bool allowTrivial = true, const bool verbose = false, 
        const bool simplePIR = false,
        const bool randomData = true, 
        const uint64_t batchSize = 1,
        const bool preproc = false,
        const bool honestHint = false
    ) : N(row_in * col_in), d(d_in), db(row_in * col_in, d_in)
    {
        //dbParams = db.computeParams(allowTrivial, verbose, simplePIR, batchSize, preproc, honestHint);
        //assert(lattice_n_in == LHE::n && "Can only use fixed lattice dimensions. See vsp/lhe.h");
        uint64_t N = row_in * col_in;
        //std::cout << "Confirming params" << std::endl;
        dbParams = db.confirmHintlessParams(N, logp_in, row_in, col_in, d_in, allowTrivial, true, simplePIR, batchSize, true, honestHint);
        lhe = LHE(dbParams.p, row_in);
        ...
        // TODO: remove this; just quickly checking if small kappa leads to incorrect decryption of Z when \ell, m is small
        //kappa = 17;
        ...
        // Already checked by confirmHintlessParams
        //if (!simplePIR){
        //    checkSISHardness();
        //    checkSISHardnessNew();
        //}
    ...
    // TODO: needs to be rethought and confirmed.
    void checkSISHardnessNew() {
        const float left = float(4*ell*ell * lhe.p) * std::sqrt(float(m));
        const float right = std::exp2(2*std::sqrt(float(lhe.n * lhe.logq * std::log2(1.005))));
        if ((left > right) || (std::log2(left) > lhe.logq)) {
            std::cout << "New SIS is not hard!\n";
            assert(false);
        }
    }
    ...
    Matrix PreprocInitNew() const; // Don't need multilimb with our new simplified preprocessing
    Matrix PreprocInitNew(uint64_t lwe_dim) const;
    Matrix64 PreprocInitNew64() const; // Don't need multilimb with our new simplified preprocessing; 64-bit version
    Matrix PreprocInitNewSecondary() const; // Used when we have the honest hint assumption
    Matrix PreprocInitNewSecondary(uint64_t lwe_dim) const; // Used when we have the honest hint assumption
    ...
    Matrix PreprocGenerateHintNew(const Matrix& A, const Matrix& D) const;
    ...
    BinaryMatrix PreprocSampleCExplicitDim(uint32_t cols) const;
    ...
    std::vector<Matrix> PreprocAnswerNew(
    ...
    void VerifyPreprocZCompressed(
        const Matrix& RZ,
        const Matrix& A_1, const Matrix& RC, const Matrix& H_1,
        const bool fake = false
    ) const;

    void VerifyPreprocZSecondary(
        const Matrix& Z,
        const Matrix& A_1, const BinaryMatrix& C, const Matrix& H_1,
        const bool fake = false
    ) const;

    void VerifyPreprocZSecondaryCompressed(
        const Matrix& RZ,
        const Matrix& A_1, const Matrix& RC, const Matrix& H_1,
        const bool fake = false
    ) const;

    Matrix PreprocRecoverZNew(
    const Matrix& H_2,
    ...
    Matrix GenerateHint(const Matrix& A, const Matrix& D, bool solinas = false, int record_bitsize = 0) const;
    ...
    Matrix QueryGivenLweSk(const Matrix& A, const uint64_t index, const Matrix& secretKey_in) const;
    Matrix QueryGivenLweSkPNNS(const Matrix& A, const uint64_t index, const Matrix& secretKey_in) const;

    std::pair<Matrix, Matrix> RandomPNNSQueryGivenLweSk(const Matrix& A, const Matrix& secretKey_in) const;
    ...
    void PreVerifyCompressed(const Matrix& u, const Matrix& v, const Matrix& rZ, const Matrix& rC, const bool fake = false) const;

    void HsVerify(const Matrix& s, const Matrix& w, const Matrix& Z, const BinaryMatrix& C, const Matrix& A, const bool fake = false) const;

    void HsVerifyCompressed(const Matrix& s, const Matrix& w, const Matrix& rZ, const Matrix& rC, const Matrix& A, const bool fake = false) const;

    void HsVerifyCompressed(const Matrix& As, const Matrix& w, const Matrix& rZ, const Matrix& rC, const bool fake = false) const;
    ...
    Matrix RecoverPNNSScores(const Matrix& hint, const Matrix& ciphertext, const Matrix& secretKey) const;
};
