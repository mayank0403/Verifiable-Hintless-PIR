...
uint64_t get_right_check_sis() {
    // delta = 1.005
    const uint64_t right_check_exp = 2*sqrt(LHE::n * LHE::logq*log2(1.005));
    //std::cout << "right check exp: " << right_check_exp << std::endl;
    // Change: changing Elem here to uint64_t because after increasing LWE n, the result now overflows Elem when Elem is uint32_t
    const uint64_t right_check = 1ULL << std::min(LHE::logq, right_check_exp);
    return right_check;
}

uint64_t get_right_check_sis_offline() {
    // delta = 1.005
    const uint64_t right_check_exp = 2*sqrt(LHE::n * (LOG_Q)*log2(1.005));
    //std::cout << "right check exp: " << right_check_exp << std::endl;
    // Change: changing Elem here to uint64_t because after increasing LWE n, the result now overflows Elem when Elem is uint32_t
    const uint64_t q_bits = (LOG_Q);
    
    const uint64_t right_check = 1ULL << std::min(q_bits, right_check_exp);
    return right_check;
}
...
bool check_slack_lwe_correctness(
    ...
    // const Elem q = get_close_to_q();
    // if (q < lower_bound) {
    std::cout << "logq_lower_bound: " << log_lower_bound << std::endl;
...
bool check_slack_lwe_correctness_new(
    const Elem p, const uint64_t m, const Elem d,
    const bool honest_hint = true, const uint64_t ell=1, 
    const bool verbose=false
) {
    constexpr double sigma = 6.4;   // fixed
    ...
    // uint64_t lower_bound = sigma * pow(p, 2) * sqrt(2*m*log(2/LWE_ERROR_PROB));
    //double log_lower_bound = log2(sigma) + 2*log2(p) + 0.5*log2(2*m*log(2/LWE_ERROR_PROB));
    double log_lower_bound = log2(sigma) + log2(p) + d + 0.5*log2(2*m*log(2/LWE_ERROR_PROB));
    if (!honest_hint)
        log_lower_bound += 1 + log2(ell);
        // lower_bound *= ell*2;
...
bool check_slack_lwe_correctness_new_online(
    const Elem p, const uint64_t m, const Elem d,
    const bool honest_hint = true, const uint64_t ell=1, 
    const bool verbose=false
) {
    ...
    if (LHE::logq < log_lower_bound) {
        std::cout << "Online logq_lower_bound: " << log_lower_bound << std::endl;
        std::cout << "Online logq: " << LHE::logq << std::endl;
        if (verbose) 
            std::cout << "ONLINE slack lwe correctness failed\n";
        return false;
    }
...
bool check_slack_lwe_correctness_new_offline(
    const Elem p, const uint64_t m, const Elem d,
    const bool honest_hint = true, const uint64_t ell=1, 
    const bool verbose=false
) {
    ...
    if (VSPIR_LHE_LOG_Q < log_lower_bound) {
        std::cout << "logq_lower_bound: " << log_lower_bound << std::endl;
        std::cout << "logq: " << VSPIR_LHE_LOG_Q << std::endl;
        if (verbose) 
            std::cout << "slack lwe correctness failed\n";
        return false;
    }
...
bool check_sis_hardness(const Elem p, const uint64_t m, const uint64_t ell, const bool verbose) {
    const uint64_t left = 4*ell*p*sqrt(m);
    const uint64_t right = get_right_check_sis();
    std::cout << "SIS Check (l < r): " << "left: " << left << ", right: " << right << std::endl;
    if (left > right) {
        std::cout << "SIS hardness failed\n";
...
bool check_sis_hardness_new_offline(const Elem p, const uint64_t m, const uint64_t ell, const bool verbose) {
    const uint64_t left = 2*ell*p*sqrt(m); // Constant should be 2 I think. Putting 4 for good measure.
    const uint64_t right = get_right_check_sis_offline();
    
    if (left > right) {
        std::cout << "Offline New SIS Check (l < r): " << "left: " << left << ", right: " << right << std::endl;
        std::cout << "Offline New SIS hardness failed. Try Lattice Estimator for close calls\n";
        return false;
    }
    return true;
}
...
Matrix Database::packDataInMatrix(const PlaintextDBParams& params, const bool verbose) const {
    ...
    // floor(log(p)) to ensure that d always fits in the Z_p
    ...
    if (floor(log2(params.p)) >= d) {
        ...
        const uint64_t complete_pt_bits = floor(log2(params.p)); // lower bound on the plaintext space
        ...
        std::cout << "Elements per Z_p:" << elems_per_Zp << " | " << complete_pt_bits << " " << d << std::endl;
...
Matrix Database::putDataInMatrixPNNS(const PlaintextDBParams& params, const bool verbose) const {
    // database consists of N entries of at most d bits
    // Here we always assume that record_bit_size < plaintext_bits. We don't pack at all here. 

    //std::cout << "packing ratio: " << double(N*d) / (double(params.m*params.ell)*log2(params.p)) << std::endl;

    assert(d < log2(params.p)); // This condition is needed for PNNS code

    // Matrix result(params.ell, params.m);
    Matrix result(params.m, params.ell);  // column-pack data to stay consistent with large-element packing
    
    // at least one entry per Zp element
    const uint64_t elems_per_Zp = 1;
    std::cout << "Elements per Z_p (expected 1):" << elems_per_Zp << std::endl;
...
PlaintextDBParams Database::confirmHintlessParams(uint64_t N, uint64_t logp, uint64_t ell, uint64_t m, uint64_t d, const bool allowTrivial, const bool verbose, const bool simplePIR, const uint64_t batchSize, const bool preproc, const bool honest_hint) const {
    //std::cout << "Inside the function confirming params" << std::endl;

    const uint64_t logq = LHE::logq;

    const uint64_t n = LHE::n;

    const uint64_t p = 1ULL << logp;

    // Limited functionality supported currently
    //assert(d == logp);
    assert(batchSize == 1);
    assert(ell % 8 == 0);

    assert(p > 0);
    //std::cout << "Done asserts before checking LWE and SIS conditions" << std::endl;
    if (!simplePIR) {
        if (check_slack_lwe_correctness_new_online(p, m, d, true, ell, verbose) == false) {
            std::cout << "\n ERROR:\n Online slack lwe correctness failed\n\n";
            assert(false);
        }
        else {
            std::cout << "Online LWE correctness slack success" << std::endl;
        }
        if (check_slack_lwe_correctness_new_offline(p, m, d, true, ell, verbose) == false) {
            std::cout << "\n ERROR:\n Offline slack lwe correctness failed\n\n";
            assert(false);
        }
        else {
            std::cout << "Offline LWE correctness slack success" << std::endl;
        }
        if (check_sis_hardness_new_offline(p, m, ell, verbose) == false) {
            std::cout << "\n ERROR:\n Offline sis hardness failed. \nTry: lattice estimator\n\n";
            assert(false);
        }
        else {
            std::cout << "Offline SIS hardness check success" << std::endl;
        } 
    } else {
            if(check_slack_lwe_correctness_new(p, m, d, true, ell, verbose)){
                std::cout << "slack lwe correctness failed\n";
                assert(false);
            }
    }
