...
Matrix simplepir_matVecMulColPacked_variableCompression_naiveMod(const PackedMatrix& a, const Matrix& b, const Elem modulus) {
    Matrix out(a.mat.rows, 1);
    ...
    for (size_t i = 0; i < aRows; i += 8)
    {
        ...
        for (size_t j = 0; j < aCols; j++)
        {
            ...
            for (size_t compInd = 0; compInd < COMPRESSION; compInd++) {
                ...
                // assert(index2 < b.rows);
                tmp += val * b.data[index2] % modulus;
                tmp2 += val2 * b.data[index2] % modulus;
                tmp3 += val3 * b.data[index2] % modulus;
                tmp4 += val4 * b.data[index2] % modulus;
                tmp5 += val5 * b.data[index2] % modulus;
                tmp6 += val6 * b.data[index2] % modulus;
                tmp7 += val7 * b.data[index2] % modulus;
                tmp8 += val8 * b.data[index2] % modulus;
                index2 += 1;
            }
            index += 1;
        }
        out.data[i] += tmp % modulus;
        out.data[i + 1] += tmp2 % modulus;
        out.data[i + 2] += tmp3 % modulus;
        out.data[i + 3] += tmp4 % modulus;
        out.data[i + 4] += tmp5 % modulus;
        out.data[i + 5] += tmp6 % modulus;
        out.data[i + 6] += tmp7 % modulus;
        out.data[i + 7] += tmp8 % modulus;
        index += aCols * 7;
    }
