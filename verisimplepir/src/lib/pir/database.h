...
struct PlaintextDBParams {
    ...
    uint64_t indexToColumn(const uint64_t i) const {
        ...
        if (floor(log2(p)) >= d) {
            ...
            std::cout << "elems per column = " << elems_per_column << std::endl;
            std::cout << "p = " << p << std::endl;
            std::cout << "d = " << d << std::endl;
            std::cout << "log2(p) = " << log2(p) << std::endl;
    ...
    uint64_t indexToColumnPNNS(const uint64_t i) const {
        // maps index to column for a query
        // get number of elements per column
    ...
    // No packing here
    uint64_t indexToRowPNNS(const uint64_t i) const {
        uint64_t row;
        row = i % ell;
        assert(row < ell);
        return row;
...
public:
    ...
    PlaintextDBParams confirmHintlessParams(uint64_t N, uint64_t p, uint64_t ell, uint64_t m, uint64_t d, const bool allowTrivial, const bool verbose, const bool simplePIR, const uint64_t batchSize, const bool preproc, const bool honest_hint) const;
    ...
    Matrix putDataInMatrixPNNS(const PlaintextDBParams& params, const bool verbose = false) const;
