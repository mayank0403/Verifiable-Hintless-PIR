...
#else
  return SampleUniformMatrixCustomBound(num_rows, num_cols, encryption_prng, SOL_NTT_PRIME-1, SOL_NTT_PRIME_BITS);
#endif
}
...
class SymmetricLweKey {
 public:
  ...
  absl::Status EncryptFromPadInPlace(Vector& plaintext, const Matrix& pad,
    ...
    plaintext += pad * key_;
    //Can't use Eigen multiplication because of prime modulus
    ...
    for (int i = 0; i < pad.rows(); i++) {
      for (int j = 0; j < pad.cols(); j++) {
        big_temp = ((ElemDouble)pad(i, j)) * ((ElemDouble)key_(j));
        big_temp = fast_mod(big_temp);
        big_temp = big_temp + (ElemDouble)plaintext(i);
        big_temp = fast_mod_max_64(static_cast<uint64_t>(big_temp));
        plaintext(i) = static_cast<Integer>(big_temp);
        //plaintext(i) += pad(i, j) * key_(j);
      }
    }
    */
   /*
    for (int i = 0; i < pad.rows(); i++) {
      for (int j = 0; j < pad.cols(); j++) {
        if (key_(j) == static_cast<Integer>(SOL_NTT_PRIME) - 1){
          big_temp = ((ElemDouble)SOL_NTT_PRIME) - ((ElemDouble)pad(i, j)) + (ElemDouble)plaintext(i);
        }
        else if (key_(j) == 1){
          big_temp = ((ElemDouble)pad(i, j)) + (ElemDouble)plaintext(i);
        }
        big_temp = fast_mod_max_64(static_cast<uint64_t>(big_temp));
        plaintext(i) = static_cast<Integer>(big_temp);
        //plaintext(i) += pad(i, j) * key_(j);
  ...
  // See function above. This just takes scaling factor direclty rather than log_scaling_factor
  template <typename Prng = rlwe::SingleThreadHkdfPrng>
  absl::Status EncryptFromPadInPlaceExplicit(Vector& plaintext, const Matrix& pad,
                                     const Integer scaling_factor,
                                     Prng* prng) const {
    if (prng == nullptr) {
      return absl::InvalidArgumentError("The prng must not be null");
    ...
    // Encodes the vector
    RLWE_RETURN_IF_ERROR(EncodeMessageInPlaceExplicit(plaintext, scaling_factor));
    // Samples the Centered binomial and adds it to the (encoded) plaintext
    RLWE_RETURN_IF_ERROR(SampleAndAddCenteredBinomialInPlace(plaintext, prng));
    // Adds pad * s to the encoded vector \Delta * m + e
    //plaintext += pad * key_;
    ...
    */
    ...
    for (int i = 0; i < pad.rows(); i++) {
      for (int j = 0; j < pad.cols(); j++) {
        big_temp = 0;
        //std::cout<<key_(j)<<std::endl;
        if (key_(j) == (static_cast<Integer>(SOL_NTT_PRIME) - (Integer)1)){
          big_temp = (Elem64)plaintext(i) + (((Elem64)SOL_NTT_PRIME) - ((Elem64)pad(i, j)));
        }
        else if (key_(j) == 1){
          big_temp = ((Elem64)pad(i, j)) + (Elem64)plaintext(i);
        }
        else {
          big_temp = (Elem64)plaintext(i);
        }
        big_temp = fast_mod_max_64(big_temp);
        plaintext(i) = static_cast<Integer>(big_temp);
        //plaintext(i) += pad(i, j) * key_(j);
      }
    }
    end = currentDateTime();
    std::cout << "[==> TIMER  <==] Client A*s time during LWE encryption: " << (end-start) << " ms | " << (end-start)/1000 << " sec" << std::endl;
  ...
  // See function above. This just takes scaling factor direclty rather than log_scaling_factor
  template <typename Prng = rlwe::SingleThreadHkdfPrng>
  absl::Status EncryptFromPadInPlaceExplicitGivenAs(Vector& plaintext, Vector& As,
                                     const Integer scaling_factor,
                                     Prng* prng) const {
    if (prng == nullptr) {
      return absl::InvalidArgumentError("The prng must not be null");
      ...
      return absl::InvalidArgumentError(absl::StrCat(
          "The plaintext size, ", plaintext.size(),
          ", does not match the number of rows of the As, ", As.size()));
    }
    // Encodes the vector
    RLWE_RETURN_IF_ERROR(EncodeMessageInPlaceExplicit(plaintext, scaling_factor));
    // Samples the Centered binomial and adds it to the (encoded) plaintext
    ...
    */
    for (int i = 0; i < As.size(); i++) {
      big_temp = 0;
      big_temp = ((Elem64)As(i)) + (Elem64)plaintext(i);
      big_temp = fast_mod_max_64(big_temp);
      plaintext(i) = static_cast<Integer>(big_temp);
    }
