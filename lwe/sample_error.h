...
static absl::Status SampleAndAddCenteredBinomialInPlace(Vector& buffer,
  ...
  Integer mid_pt = 1 << (sizeof(Integer)*8 - 1);
  Integer magnitude;
  for (unsigned int i = 0; i < num_coeffs; i++) {
    if (buffer[i] >= mid_pt) {
      //Negative value
      magnitude = buffer[i] * (-1);
      buffer[i] = (Integer)SOL_NTT_PRIME - magnitude;
    }
  }
...
static absl::StatusOr<Vector> SampleUniformTernary(int num_coeffs, Prng* prng) {
  ...
  for (int i = 0; i < coeffs.size(); i++) {
    if (coeffs[i] == minus_one) {
      // Negative value = -1
      //magnitude = coeffs[i] * (-1);
      coeffs[i] = (Integer)SOL_NTT_PRIME - 1;
      }
  }
...
static absl::Status SampleUniformVectorInPlace(Vector& buffer, Prng* prng) {
  int num_coeffs = buffer.size();
  if (num_coeffs % 2 != 0 && sizeof(Integer) <= 4) {
    return absl::InvalidArgumentError(
        absl::StrCat("The size of `buffer`, ", num_coeffs, ", must be even."));
  }
  if (prng == nullptr) {
  ...
  else if (sizeof(Integer) == 8) { // 64-bit values
    for (int i = 0; i < num_coeffs; i += 1) {
      RLWE_ASSIGN_OR_RETURN(uint64_t sample, prng->Rand64());
      buffer[i] = (static_cast<Integer>(sample));
    }
    return absl::OkStatus();
  }
  else {
    std::cout << "\nERROR: Integer size not supported\n" << std::endl;
    return absl::InvalidArgumentError("Integer size not supported");
  }
}

// Non-constant time function. Ok for security because called to generate
// LWE pad which everyone in the system knows.
// Samples a vector of uniforms without allocating
template <typename Prng = rlwe::SingleThreadHkdfPrng>
static absl::Status SampleUniformVectorInPlaceCustomBound(Vector& buffer, Prng* prng, Integer max_inc, int max_bits) {
  int num_coeffs = buffer.size();
  if (num_coeffs % 2 != 0 && sizeof(Integer) <= 4) {
    return absl::InvalidArgumentError(
        absl::StrCat("The size of `buffer`, ", num_coeffs, ", must be even."));
  ...
  Integer max_bits_mask = (Integer{1} << max_bits) - 1;
  ...
  if (sizeof(Integer) == 4) { // 32-bit values
    ...
    for (int i = 0; i < num_coeffs; i += 2) {
      RLWE_ASSIGN_OR_RETURN(uint64_t sample, prng->Rand64());
      v1 = (static_cast<Integer>(sample & low_mask)) & max_bits_mask;
      v2 = (static_cast<Integer>(sample >> 32)) & max_bits_mask;
      while (v1 > max_inc || v2 > max_inc) {
        //resample_count++;
        //std::cout << "Resampling " << v1 << " " << v2 << " " << max_inc << " " << max_bits << " " << max_bits_mask << std::endl;
        RLWE_ASSIGN_OR_RETURN(uint64_t sample, prng->Rand64());
        v1 = (static_cast<Integer>(sample & low_mask)) & max_bits_mask;
        v2 = (static_cast<Integer>(sample >> 32)) & max_bits_mask;
      }
    ...
    for (int i = 0; i < num_coeffs; i += 1) {
      RLWE_ASSIGN_OR_RETURN(uint64_t sample, prng->Rand64());
      v = (static_cast<Integer>(sample)) & max_bits_mask;
      while (v > max_inc) {
        RLWE_ASSIGN_OR_RETURN(uint64_t sample, prng->Rand64());
        v = (static_cast<Integer>(sample)) & max_bits_mask;
      }
...
// contained in.
template <typename Prng = rlwe::SingleThreadHkdfPrng>
static absl::StatusOr<Matrix> SampleUniformMatrixCustomBound(int num_rows, int num_cols,
                                                  Prng* prng, Integer max_inc, int max_bits) {
  if (num_rows < 0) {
    return absl::InvalidArgumentError("num_rows must be non-negative.");
  }
  if (num_cols < 0) {
