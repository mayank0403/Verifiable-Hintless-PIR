...
class Database {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<Database>>
    CreateBSGS(
    const RlweParameters<RlweInteger>& rlwe_params,
    const RnsContext* rns_context,
    const std::vector<std::vector<RlweInteger>>& data);
  ...
  absl::Status PreprocessBSGS(absl::Span<const RnsPolynomial> pad_rotated_queries);

  absl::Status SetPreprocessedFinalPadsBSGS(std::vector<RnsPolynomial> pads);
  ...
  absl::StatusOr<std::vector<std::vector<RnsCiphertext>>> InnerProductWithPreprocessedPadsBSGS(
      absl::Span<const RnsCiphertext> ct_rotated_queries) const;
  ...
  std::vector<std::vector<RnsPolynomial>>& GetBabyInnerProductsPad() {return pad_inner_products_baby_;};
 ...
 private:
  ...
  explicit Database(const RnsContext* rns_context,
                    std::vector<const PrimeModulus*> moduli, Encoder encoder,
                    std::vector<std::vector<std::vector<RnsPolynomial>>> diagonals_bsgs)
      : rns_context_(rns_context),
        moduli_(std::move(moduli)),
        encoder_(std::move(encoder)),
        diagonals_bsgs_(std::move(diagonals_bsgs)) {}
  ...
  // Diagonals for BSGS (are nested to another level over diagonals_)
  std::vector<std::vector<std::vector<RnsPolynomial>>> diagonals_bsgs_;
  ...
  std::vector<std::vector<RnsPolynomial>> pad_inner_products_baby_;

  std::vector<RnsPolynomial> pad_inner_products_giant_;
};
