...
class Server {
 public:
  ...
  absl::StatusOr<LinPirResponse> HandleRequestBSGS(
    const ::rlwe::SerializedRnsPolynomial& proto_ct_query_b,
    const google::protobuf::RepeatedPtrField<::rlwe::SerializedRnsPolynomial>&
 ...
 private:
  ...
  explicit Server(RlweParameters<RlweInteger> params,
                  std::string prng_seed_ct_pad, std::string prng_seed_gk_pad_baby, std::string prng_seed_gk_pad_giant,
                  const RnsContext* rns_context,
                  std::vector<const PrimeModulus*> rns_moduli,
                  RnsGadget rns_gadget, RnsErrorParams rns_error_params,
                  std::vector<Database<RlweInteger>*> databases)
  ...
  // Used by BSGS
  std::vector<RnsPolynomial> ct_pads_baby_;
  std::vector<std::vector<RnsPolynomial>> ct_sub_pad_baby_digits_;
  std::vector<std::vector<std::vector<RnsPolynomial>>> ct_pads_giant_; // extra dimension because outer layer is database count and then also number of rows of hint considered at once as a chunk
  std::vector<std::vector<std::vector<std::vector<RnsPolynomial>>>> ct_sub_pad_giant_digits_;
  std::vector<RnsPolynomial> gk_pads_baby_;
  std::vector<RnsPolynomial> gk_pads_giant_;

  std::vector<std::vector<RnsPolynomial>> pad_inner_products_giant_;
};
